...
const char* model__path = NULL;
int max__seq = 0;
const char* dit__path = NULL;
bool use__fa = true;
bool use__fsm = true;
...
///////////////////////  PPFIX ///////////////////////////////////////
    model__path = params->model_path;
    max__seq = params->max_seq ;
    dit__path = params->dit_path ;
    use__fa = params->use_fa ;
///////////////////////////////////////////////////////////////////////
...
void ace_fsq_free_(AceUnderstand * ctx) {
    if (!ctx) {
        return;
    }
    if (ctx->have_fsq) {
    ...
    fprintf(stderr, "[FREE]  FSQ \n");
}

void ace_vae_free_(AceUnderstand * ctx) {
    if (!ctx) {
        return;
    }
    if (ctx->have_vae_enc) {
...
void ace_lm_free_(AceUnderstand * ctx) {
    if (!ctx) {
        return;
    }
    if (ctx->have_lm && ctx->owns_lm) {
...
void ace_lm_load_(AceUnderstand * ctx) {

        // owned: load our own copy from GGUF (standalone CLI path)
        if (!load_bpe_from_gguf(&ctx->bpe_storage, model__path)) {
            delete ctx;            
        }
        if (!qw3lm_load(&ctx->model_storage, model__path, max__seq, 1)) {
            delete ctx;
        }
        ctx->model_storage.use_flash_attn = use__fa;
        fprintf(stderr, "[Understand-Load] LM: \n");
